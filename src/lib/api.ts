...
// This data is used ONLY when Supabase is not configured or fails to connect.
...
// ====================================================================
// --- ROBUST LOGIN & USER CHECK FUNCTIONS ---
// ====================================================================
...
export async function loginUser(phone: string, role: UserRole): Promise<{ success: boolean; user?: User; message?: string }> {
    ...
    const useLocalData = () => {
        console.warn(`DEV_MODE: Using local fallback data for login. Role: ${role}`);
        ...
        if (foundUser) {
            ...
            return { success: false, message: 'کاربری یافت نشد. لطفا ابتدا ثبت‌نام کنید.' };
        }
    };
    ...
    try {
        ...
        if (error) {
            ...
            throw error; // Rethrow other DB errors to be caught below
        }
        ...
        console.error("Supabase connection failed during login. Falling back to local data.", e);
        return useLocalData();
    }
}
...
export async function checkIfUserExists(phone: string): Promise<boolean> {
    ...
    const useLocalData = () => {
        console.warn("DEV_MODE: Using local fallback data for user existence check.");
...
// The rest of the functions will rely on the handleSupabaseRequest helper,
// which will throw an error if Supabase is not configured. This is acceptable
// as these are less critical than login/registration.
...
export async function updateProviderDetails(phone: string, details: { name: string; service: string; bio: string; }): Promise<Provider> {
    if (!supabase) throw new Error("Cannot update provider details: Database not configured");
    const normalizedPhone = normalizePhoneNumber(phone);
...
async function uploadImageFromBase64(base64Data: string, phone: string, folder: 'portfolio' | 'profile'): Promise<string> {
    if (!supabase) return "https://placehold.co/400x400.png";
    const normalizedPhone = normalizePhoneNumber(phone);
...
export async function addReview(reviewData: Omit<Review, 'id' | 'created_at'>): Promise<Review> {
    if (!supabase) throw new Error("Cannot add review: Database not configured");
...
export async function createAgreement(provider: Provider, customer: User): Promise<Agreement> {
    if (!supabase) throw new Error("Cannot create agreement: Database not configured");
...
export async function confirmAgreement(agreementId: number): Promise<Agreement> {
    if (!supabase) throw new Error("Cannot confirm agreement: Database not configured");
