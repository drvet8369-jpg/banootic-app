...
import { MapPin, User, AlertTriangle, PlusCircle, Trash2, Camera, Edit, Save, XCircle, Sparkles, Loader2 } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  const handleGenerateBio = async () => {
    if (!editedData.name || !editedData.service) {
      toast({ title: "اطلاعات ناقص", description: "لطفاً ابتدا نام و نوع خدمت را وارد کنید.", variant: 'destructive' });
      return;
    }
    setIsGeneratingBio(true);
    toast({ title: "در حال پردازش...", description: "دستیار هوش مصنوعی در حال نوشتن بیوگرافی است." });
    ...
    if (result.error) {
        toast({ title: "خطا در تولید بیوگرافی", description: result.error, variant: 'destructive' });
    } else if (result.biography) {
        setEditedData(prev => ({...prev, bio: result.biography!}));
        toast({ title: "موفق", description: "بیوگرافی جدید در کادر زیر قرار گرفت. می‌توانید آن را ویرایش کنید." });
    }
